...
function autoGenerateDocumentacion(idioma) {
  ...
  saveDocStateToLocalStorage(); // 🆕 guardamos al regenerar
}
...
function attachDocumentacionHandlers() {
  ...
  container.querySelectorAll("[data-doc-lang]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      // autoGenerate ya guarda en localStorage
  ...
  container.querySelectorAll(".doc-section-textarea").forEach((ta) => {
    ta.addEventListener("input", () => {
      ...
      saveDocStateToLocalStorage(); // 🆕 guardamos cambios de texto
    });
  });
...
function saveDocCustomBlock() {
  ...
  saveDocStateToLocalStorage(); // 🆕 guardamos bloque custom
  closeDocCustomModal();
  renderDocumentacionView();
}
...
async function ensureDocMediaLoaded() {
  ...
  try {
    ...
    if (uid) query = query.where("uid", "==", uid); // solo where, sin orderBy

    const snap = await query.limit(200).get(); // sin índice compuesto
...
async function handleMediaUpload(files, options = {}) {
  ...
  saveDocStateToLocalStorage(); // 🆕 (aunque lo importante son ids, no el binario)
  renderDocumentacionView();
}
