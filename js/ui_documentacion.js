...
// ⚠️ Nombre ÚNICO para no chocar con ui_doc_gestion.js
const DOC_DOCUMENTACION_STORAGE_KEY = "docState_documentacion_v1";
...
(function loadDocStateFromLocalStorage() {
  try {
    const raw =
      window.localStorage &&
      localStorage.getItem(DOC_DOCUMENTACION_STORAGE_KEY);
    if (!raw) return;
...
async function renderDocumentacionView() {
  ...
  // Siempre cargamos media al entrar, para evitar tener que refrescar
...
function renderSectionMediaHTML(sectionKey) {
  ...
  return ids
    .map(function (id) {
      ...
      const isImage =
        cat === "imagen" || m.type === "image" || mime.indexOf("image/") === 0;
...
function renderDocMediaLibraryHTML() {
  ...
  console.log(
    "[DOC] renderDocMediaLibraryHTML – total media:",
    allMedia.length
  );
  ...
  const imagesOnly = allMedia.filter(function (m) {
    return m && m.id && m.url && isImageItem(m);
  });

  console.log(
    "[DOC] renderDocMediaLibraryHTML – imagesOnly:",
    imagesOnly.length
  );

  // Si hay fotos, usamos solo fotos; si no hay, usamos toda la media como fallback
  let baseList = imagesOnly.length ? imagesOnly : allMedia.slice();
  ...
  const visible = baseList.filter(function (m) {
    return m && m.id && m.url;
  });

  console.log("[DOC] renderDocMediaLibraryHTML – visible:", visible.length);
    ...
    visible
      .map(function (m) {
        ...
        return (
          ...
          m.id +
          '" title="Ver documento">👁 Ver</button>' +
          "  </div>" +
          "</div>"
        );
      })
...
async function ensureDocMediaLoaded() {
  ...
  if (!db) {
    console.warn("[DOC] ensureDocMediaLoaded: Firestore no disponible");
    return;
  }
...
function saveDocStateToLocalStorage() {
  try {
    ...
    localStorage.setItem(DOC_DOC_STORAGE_KEY, JSON.stringify(toSave));
  } catch (e) {
    console.error(
      "Error guardando estado de documentación en localStorage:",
      e
...
function renderDocMediaLibraryHTML() {
  ...
  function isImageItem(m) {
    ...
    const isByCat = cat === "imagen";
    const isByMime = mime.indexOf("image/") === 0;
    const isByType = type === "image";
    const isByExt =
      url.endsWith(".png") ||
      url.endsWith(".jpg") ||
      url.endsWith(".jpeg") ||
    ...
    return isByCat || isByMime || isByType || isByExt;
  }

  let baseList = allMedia.filter(function (m) {
    return m && m.id && m.url && isImageItem(m);
  });
...
async function renderDocumentacionView() {
  ...
  // ⬇️ Ahora esperamos a tener la media cargada
  await ensureDocMediaLoaded();
...
function renderDocMediaLibraryHTML() {
  ...
  // Helper robusto para saber si el item es una imagen
  function isImageItem(m) {
    if (!m) return false;
  ...
  // Solo imágenes (según heurística anterior)
  let baseList = allMedia.filter(function (m) {
    return m && m.id && m.url && isImageItem(m);
  });
...
async function ensureDocMediaLoaded() {
  // Si ya está cargado y tenemos datos, no recargamos
  if (appState.documentacion.mediaLoaded && appState.documentacion.mediaLibrary?.length) {
    return;
  }
