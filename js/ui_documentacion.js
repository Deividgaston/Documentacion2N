...
function autoGenerateDocumentacion(idioma) {
  ...
  DOC_SECTION_ORDER.forEach((key) => {
    if (key === "presentacion_empresa") {
      // Texto base de presentación (editable + IA) con texto completo 2N
      nuevas[key] =
        d.secciones[key] ||
...
async function exportarPDFTecnico() {
  ...
  // Anexo fichas técnicas (dejamos fuentes como estaban)
...
// ======================================================
// PDF COMERCIAL (títulos y texto de secciones 20 / 11)
// ======================================================
...
async function exportarPDFComercial() {
  ...
  function drawSalesforceCard({
    ...
    const bodyH = lines.length * 5.0; // un poco más alto para cuerpo 11pt
  ...
  // Página 2 – Presentación de empresa (20/11)
  const includePresentacion =
  ...
  // Página 3 – Resumen + Sistema (cards 20/11)
  doc.addPage();
  ...
  // Página 4 – Servicios / Infraestructura / Otros (cards 20/11)
  ...
  container.innerHTML = `
    <div class="doc-layout">
      ...
      <div class="doc-main-grid">
        ...
        <!-- Columna derecha: documentación gráfica + fichas -->
