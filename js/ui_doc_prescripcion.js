...
function encodeWin1252(str) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    // Latin-1 directo (0xA0..0xFF) coincide con Win-1252
    if (code >= 0xA0 && code <= 0xFF) {
      out[i] = code;
      continue;
    }

    // Extra Win-1252 (0x80..0x9F) más comunes
    // (si tu texto no los usa, no pasa nada)
    const map = {
      0x20AC: 0x80, // €
      0x201A: 0x82, // ‚
      0x0192: 0x83, // ƒ
    ...
    // Fallback: si hay algún carácter raro fuera de CP1252, lo sustituimos por '?'
    out[i] = 0x3F;
  }
...
function downloadBc3File(content, filename) {
  ...
  // puedes dejar octet-stream o texto; ambos suelen ir bien
