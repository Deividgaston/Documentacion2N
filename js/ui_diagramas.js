...
function diagExportDxf() {
  ...
  // ✅ Construcción robusta “por líneas” (prioriza secciones de la plantilla si existen)
  const outLines = [];

  // Preferimos HEADER/CLASSES/TABLES/OBJECTS de la plantilla para máxima compatibilidad AutoCAD LT
  const headerText = appState.diagramas.dxfHeaderSection || "";
  const classesText = appState.diagramas.dxfClassesSection || "";
  const tablesText = appState.diagramas.dxfTablesSection || "";
  const objectsText = appState.diagramas.dxfObjectsSection || "";

  const header = headerText ? headerText : _buildMinimalHeaderSection();
  const tables = tablesText ? tablesText : _buildMinimalTablesSection();
  ...
  // OBJECTS (si existe en plantilla, mejora compatibilidad con estilos/plotstyles)
  if (objectsText) outLines.push(..._dxfSectionToLines(objectsText));
...
window.diagExportDxf = diagExportDxf;// js/ui_diagramas.js
// Vista: DIAGRAMAS (IA)
// V2.10 (Hito 16+):
// - ✅ Export SVG (MAESTRO) estable (líneas + nodos + textos) basado en coords del preview
// - ✅ Mantiene Export DXF (ASCII) SIN atributos (opcional) reutilizando secciones de plantilla
