...
function _buildSchematicCoordsFromResult(result) {
  ...
  function _touchDevY(zoneKey, y) {
    ...
    if (!(zoneKey in devMinY)) devMinY[zoneKey] = y;
    if (!(zoneKey in devMaxY)) devMaxY[zoneKey] = y;
    devMinY[zoneKey] = Math.min(devMinY[zoneKey], y);
  ...
  for (const p of placements) {
    ...
    const m = manual[p.id];
    if (m && Number.isFinite(m.x) && Number.isFinite(m.y)) {
      map.set(p.id, { x: m.x, y: m.y, label, kind: "placement", zone, qty });
      _touchDevY(zone, m.y);
      continue;
    }
    ...
    zoneDeviceCount[zone] = idx + 1;

    const y = y0 + idx * rowH;

    map.set(p.id, { x: bx, y, label, kind: "placement", zone, qty });
    _touchDevY(zone, y);
  }
...
function _bindPreviewWindowListeners(svg) {
  ...
  function _commitAndStop() {
    ...
    _diagDrag.nodeEl = null;
    _diagDrag.baseX = null;
  ...
  s._previewPointerMoveHandler = (ev) => {
    if (!_diagDrag.active || !_diagDrag.nodeId || !_diagDrag.nodeEl) return;
