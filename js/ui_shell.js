...
function normalizeCapabilities(caps, role) {
  ...
  // Caso B (NUEVO): solo v2 -> derivar legacy (para compat con código viejo)
  if (caps && caps.pages && (!caps.views || !caps.features)) {
    ...
    if (typeof pages.prescripcion === "string") out.views.prescripcion = pages.prescripcion !== "none";
...
function normalizeCapabilities(caps, role) {
  ...
  if (caps && caps.views && (!caps.pages || typeof caps.pages !== "object")) {
    ...
    Object.keys(caps.views).forEach((k) => {
      const allowed = !!caps.views[k];
      if (k === "tarifas") pages.tarifas = allowed ? "view" : "none";
      else if (k === "documentacion") pages.documentacion = allowed ? (out.features?.docModo === "technical" ? "technical" : "commercial") : "none";
      else if (k === "prescripcion") pages.prescripcion = allowed ? "view" : "none";
      else pages[k] = allowed;
    });
    out.pages = pages;
